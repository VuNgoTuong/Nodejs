...
async function seedUser () {
  try {
    ...
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'admintch' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'admin123',
        'username':'admintch',
      ...
      await dbService.updateByQuery(model.user, { 'username':'admin123' }, userToBeInserted);
    }
    userToBeInserted = await dbService.findOne(model.user,{ 'username':'usertch' });
    if (!userToBeInserted) {  
      userToBeInserted = {
        'password':'user123',
        'username':'usertch',
      ...
      userToBeInserted.password = await bcrypt.hash(userToBeInserted.password, 8);
      await dbService.updateByQuery(model.user, { 'username':'usertch' }, userToBeInserted);
    }
...
async function seedRouteRole () {
  try {
    const routeRoles = [ 
      ...
      {
        route: '/client/api/v1/itemvariant/create',
        role: 'User',
        method: 'POST'
      },
      {
        route: '/client/api/v1/itemvariant/list',
        role: 'User',
        method: 'POST'
      },
      {
        route: '/client/api/v1/itemvariant/addBulk',
        role: 'User',
        method: 'POST'
      }
    ];
