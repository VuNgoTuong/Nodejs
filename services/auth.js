 ...
 */
const generateToken = async ([user, username], secret) => {
  return jwt.sign(
    {
      id: user.id,
      username: username,
    ...
    secret,
    { expiresIn: JWT.EXPIRES_IN * 6 }
  );
};


const refToken = async ([user, username], secret) => {
  const refresh_Token = jwt.sign(
    ...
    secret,
    { expiresIn: JWT.EXPIRES_IN * 60000 }
  );
...
auth.loginUser = async (username, password, platform, roleAccess) => {
  try {
      ...
      if (userAuth.loginReactiveTime) {
        ...
        if (limitTime > now) {
          ...
          if (!(limitTime > expireTime)) {
            return {
              flag: true,
              data: `you have exceed the number of limit.you can login after ${common.getDifferenceOfTwoDatesInTime(
                now,
                limitTime
              )}.`,
            };
          ...
          userAuth = await dbService.findOne(model.userAuthSettings, {
            userId: user.id,
          });
        }
      } else {
    ...
    if (platform == PLATFORM.ADMIN) {
      ...
      token = await generateToken([userData, username], JWT.ADMIN_SECRET);
      reftoken = await refToken([userData, username], JWT.ADMIN_SECRET);
      ...
      token = await generateToken([userData, username], JWT.DEVICE_SECRET);
      reftoken = await refToken([userData, username], JWT.DEVICE_SECRET);
      ...
      token = await generateToken([userData, username], JWT.CLIENT_SECRET);
      reftoken = await refToken([userData, username], JWT.CLIENT_SECRET);
    }
    ...
    // console.log(userToReturn);   //data user
    return {
      flag: false,
      data: userToReturn,
    };
...
auth.changePassword = async (params) => {
  try {
    ...
    let updatedUser = dbService.updateByPk(model.user, user.id, {
      password: password,
    });
    if (!updatedUser) {
      return {
...
auth.sendResetPasswordNotification = async (user) => {
  ...
  try {
    ...
    if (FORGOT_PASSWORD_WITH.LINK.email) {
      let updatedUser = await dbService.findOne(model.user, { id: user.id });
      let mailObj = {
...
auth.resetPassword = async (userId, newPassword) => {
  try {
    ...
    let updatedUser = await dbService.updateByPk(model.user, userId, {
